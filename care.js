...
function careTagMap(r){const m={};for(const u of (r.before_photos||[]))m[careUrl(u)]='before';for(const u of (r.after_photos||[]))m[careUrl(u)]='after';return m}
function careCompare(r) {
  const before=(r.before_photos||[]).map(careUrl).filter(Boolean), after=(r.after_photos||[]).map(careUrl).filter(Boolean);
  if(!before.length||!after.length) return '';
  return `<div class="compare"><figure><img src="${esc(before[0])}" alt="케어 전" loading="lazy"><figcaption>케어 전</figcaption></figure><figure><img src="${esc(after[0])}" alt="케어 후" loading="lazy"><figcaption>케어 후</figcaption></figure></div>`;
}
...
function careLocalize(text,store){
  // 본사(HQ)는 원문 그대로. 지점은 '세탁플랜' → 지점명(예: 세탁플랜 내리점, 신길옷세탁)으로 자동 치환.
  text=String(text??''); if(!store||store.is_hq||!store.name||!text)return text;
  const name=String(store.name).trim(); if(!name||name==='세탁플랜')return text;
  const tok='\u0000';
  return text.split(name).join(tok).split('세탁플랜').join(name).split(tok).join(name);
}
function renderCarePage(payload) {
  ...
  app.innerHTML=`<div class="care-public"><section class="care-hero"><p class="eyebrow">세탁플랜 · 케어 이력</p><h1>${esc(item.brand||'브랜드 미등록')}</h1><p class="model">${careText(item.item_type)}</p><p class="care-code">관리번호 <strong>${esc(item.public_code)}</strong></p><dl class="care-stats"><div><dt>누적 케어</dt><dd>${records.length}<small>회</small></dd></div><div><dt>첫 케어</dt><dd class="stat-text">${first?careFmt(first):'—'}</dd></div><div><dt>최근 케어</dt><dd class="stat-text">${records[0]?careFmt(records[0].received_on):'—'}</dd></div></dl>${settings.warranty_text?`<div class="greeting"><p>${esc(careLocalize(settings.warranty_text,store))}</p><span>— ${esc(store.is_hq===false&&store.name?store.name:(settings.business_name||'세탁플랜'))}</span></div>`:''}</section><section class="care-section"><div class="section-heading"><h2>케어 기록</h2><span class="muted">${records.length}건</span></div><ul class="care-list">${records.map(r=>{const compare=careCompare(r);const skip=new Set([...(r.before_photos||[]),...(r.after_photos||[])].map(careUrl));const photos=carePhotoList(r).filter(u=>!compare||!skip.has(u)).slice(0,3);const services=careServices(r);return `<li><time>${careFmt(r.received_on)}</time><div class="care-body"><p class="care-services">${services.length?esc(services.join(' · ')):'케어 완료'}</p>${r.staff_comment?`<p class="care-comment">${esc(careLocalize(r.staff_comment,store))}</p>`:''}${compare}${careGallery(photos,'케어 사진')}</div></li>`}).join('')||'<li class="muted">아직 등록된 케어가 없습니다.</li>'}</ul></section><footer class="care-footer"><p class="eyebrow">NEXT CARE</p><h2>다음 케어도, 안심하고.</h2>${booking?`<a class="booking" href="${esc(booking)}" target="_blank" rel="noopener noreferrer">네이버로 예약하기 ↗</a>`:`<div class="store-block"><p class="store-label">맡기신 매장</p><p class="store-line">${esc(store.name||'세탁플랜')}</p>${store.phone?`<a class="booking store-call" href="tel:${esc(String(store.phone).replace(/[^0-9+]/g,''))}">전화하기 ${esc(store.phone)}</a>`:''}</div>`}<div class="business">${settings.business_name?`<strong>${esc(settings.business_name)}</strong>`:''}${settings.business_owner?`<p>대표 ${esc(settings.business_owner)}</p>`:''}${settings.business_number?`<p>사업자등록번호 ${esc(settings.business_number)}</p>`:''}${settings.business_address?`<p>${esc(settings.business_address)}</p>`:''}</div></footer></div>`;
}
...
function carePhotoPicker(urls,tags={}) {
  const safe=(urls||[]).map(careUrl).filter(Boolean);
  return `<div class="photo-picker" id="existing-photos">${safe.map((url,i)=>`<figure data-url="${esc(url)}"><img src="${esc(url)}" alt="기존 사진 ${i+1}" loading="lazy"><button type="button" class="photo-x del-existing" data-url="${esc(url)}" aria-label="사진 삭제">×</button><button type="button" class="photo-tag tag-existing ${tags[url]||''}" data-url="${esc(url)}">${TAG_LABEL[tags[url]||'']}</button></figure>`).join('')}</div>`;
}
function bindTagToggles(scope,tags){scope.querySelectorAll('.tag-existing').forEach(b=>b.onclick=()=>{const url=b.dataset.url;tags[url]=careNextTag(tags[url]||'');b.className='photo-tag tag-existing '+tags[url];b.textContent=TAG_LABEL[tags[url]]})}
function carePhotoInput(id) {
  return `<div class="photo-add"><label class="button secondary photo-add-btn" for="${id}">＋ 사진 추가</label><input type="file" accept="image/*" multiple id="${id}" hidden><span class="muted" id="${id}-count"></span></div><p class="muted tag-help">완성품만 찍으면 그대로 두세요. 전·후 비교를 올리려면 사진 아래 버튼을 눌러 <b>케어 전</b> / <b>케어 후</b>를 지정하세요.</p><div class="photo-picker pending" id="${id}-pending"></div>`;
}
function bindPhotoInput(scope,id,pending) {
  ...
  const draw=()=>{grid.innerHTML=pending.map((f,i)=>`<figure><img src="${f._url}" alt="새 사진 ${i+1}"><button type="button" class="photo-x del-pending" data-i="${i}" aria-label="사진 빼기">×</button><button type="button" class="photo-tag tag-pending ${f._tag||''}" data-i="${i}">${TAG_LABEL[f._tag||'']}</button></figure>`).join('');count.textContent=pending.length?`새 사진 ${pending.length}장 (저장 시 업로드)`:'';grid.querySelectorAll('.del-pending').forEach(b=>b.onclick=()=>{const f=pending.splice(+b.dataset.i,1)[0];try{URL.revokeObjectURL(f._url)}catch{}draw();});grid.querySelectorAll('.tag-pending').forEach(b=>b.onclick=()=>{const f=pending[+b.dataset.i];f._tag=careNextTag(f._tag||'');draw();});};
  input.onchange=()=>{for(const f of input.files){f._url=URL.createObjectURL(f);f._tag='';pending.push(f);}input.value='';draw();};
  draw();
}
...
function careEditor(item,r={}) {
  ...
  out.querySelectorAll('.del-existing').forEach(b=>b.onclick=async()=>{if(!confirm('이 사진을 지금 바로 삭제할까요?'))return;const url=b.dataset.url;const next=existing.filter(u=>u!==url);if(r.id){const nb=(r.before_photos||[]).filter(u=>u!==url),na=(r.after_photos||[]).filter(u=>u!==url);const res=await sb.from('service_records').update({photos:next,before_photos:nb,after_photos:na}).eq('id',r.id);if(res.error){alert('삭제 실패: '+res.error.message);return;}r.photos=next;r.before_photos=nb;r.after_photos=na;}existing=next;b.closest('figure').remove();});
