...
function careDetails(r) {
  ...
  const photos=carePhotoList(r);
  return `<section class="gallery"><div class="section-sub"><h3>케어 사진</h3><span class="muted">${photos.length}장</span></div>${careGallery(photos)}</section><div class="detail-grid"><section><h3>입고 검수</h3>${checks.length?`<ul class="inspection">${checks.map(c=>`<li><div><strong>${esc(c.name)}</strong><span class="tag">${esc(c.status)}</span></div>${c.memo?`<p>${esc(c.memo)}</p>`:''}</li>`).join('')}</ul>`:'<p class="muted">검수 내용이 아직 등록되지 않았습니다.</p>'}</section><section><h3>적용 공정</h3>${processes.length?`<ol class="processes">${processes.map(p=>`<li><strong>${esc(p.name)}</strong>${p.description?`<p>${esc(p.description)}</p>`:''}</li>`).join('')}</ol>`:'<p class="muted">공정 등록 예정</p>'}</section></div><section class="safety"><h3>사용 세제 · 안전 정보</h3><p>${careText(r.detergent_info)}</p><p class="muted">${r.safety_info?esc(r.safety_info):'안전 정보가 아직 등록되지 않았습니다.'}</p></section><section class="comment"><h3>담당자 코멘트</h3><p>${r.staff_comment?esc(r.staff_comment):'담당자 코멘트가 아직 등록되지 않았습니다.'}</p><span class="muted">담당 ${careText(r.staff_name)}</span></section><div class="next-care"><span>다음 권장 케어일</span><strong>${r.next_care_on?esc(r.next_care_on):'상담 후 안내'}</strong></div>`;
}
...
function renderCarePage(payload) {
  const item=payload.item, records=careSort(payload.records||[]);
  const current=records[0]||{}, settings={...(payload.settings||{})};
...
function carePhotoPicker(urls) {
  const safe=(urls||[]).map(careUrl).filter(Boolean);
  return safe.length?`<p class="muted">기존 사진 ${safe.length}장 · 체크를 해제하면 저장할 때 삭제됩니다.</p><div class="photo-picker">${safe.map((url,i)=>`<label><input type="checkbox" class="keep-photo" value="${esc(url)}" checked><img src="${esc(url)}" alt="기존 사진 ${i+1}" loading="lazy"></label>`).join('')}</div>`:'<p class="muted">등록된 사진이 없습니다.</p>';
}
function careExisting(item,out) {
  const records=careSort(item.service_records||[]);
  out.innerHTML=`<article class="record"><strong>${esc(item.public_code)}</strong><p>${careText(item.item_type)} · ${careText(item.brand)} · ${careText(item.model_name)}</p><p class="muted">관리자용 고객 정보: ${careText(item.customer_name)} · ${careText(item.customer_phone)}</p>${records.length?`<ul class="record-list">${records.map((r,i)=>`<li><span>${esc(r.received_on||'날짜 미등록')} · 사진 ${carePhotoList(r).length}장 · ${esc((r.processes?.length?r.processes.map(p=>p.name):r.services||[]).join(', ')||'공정 미등록')}</span><button data-edit-care="${i}">사진 / 기록 수정</button></li>`).join('')}</ul>`:'<p class="muted">아직 케어 기록이 없습니다.</p>'}<button id="add-care">새 케어 기록 추가</button>${labelButton(item.public_code)}</article>`;
...
function careEditor(item,r={}) {
  ...
  out.innerHTML=`<section class="card"><h2>${esc(item.public_code)} · ${r.id?'사진 / 기록 수정':'새 케어 기록'}</h2><p class="notice">공개 항목에 고객 이름·전화번호를 적지 마세요. 사진도 개인정보가 없는 이미지를 사용하세요.</p>${careInput('care-received','케어일',r.received_on||careDate(),'date')}<div class="grid"><div>${careInput('edit-category','카테고리 (유모차 / 카시트)',item.item_type)}</div><div>${careInput('care-grade','케어등급 (제품 전체에 적용)',item.care_grade)}</div></div><div class="grid"><div>${careInput('edit-brand','브랜드',item.brand)}</div><div>${careInput('edit-model','모델명',item.model_name)}</div></div><h3>사진</h3>${carePhotoPicker(carePhotoList(r))}<label for="care-photos">사진 추가 (여러 장 선택 가능)</label><input type="file" accept="image/*" multiple id="care-photos"><p id="photo-preview" class="muted"></p><h3>기록</h3>${careExtraFields(r,'edit')}${careArea('care-private','내부 메모 (관리자 전용)',r.notes)}<button id="save-care">저장</button><button class="secondary" id="cancel-care">돌아가기</button><p id="care-message" role="status"></p></section>`;
  ...
  out.querySelector('#save-care').onclick=async()=>{
    ...
    try {
      ...
      const record={...extra,item_id:item.id,received_on:received,completed_on:received,status:'완료',notes:out.querySelector('#care-private').value,photos,before_photos:[],after_photos:[],services:extra.processes.length?extra.processes.map(p=>p.name):(r.services||[])};
      ...
      const itemValues={care_grade:out.querySelector('#care-grade').value.trim(),item_type:out.querySelector('#edit-category').value.trim(),brand:out.querySelector('#edit-brand').value.trim(),model_name:out.querySelector('#edit-model').value.trim()};const update=await sb.from('items').update(itemValues).eq('id',item.id).select('id').single();if(update.error)throw update.error;
