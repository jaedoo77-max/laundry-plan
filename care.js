...
function carePhotos(urls,label) {
  const safe = (urls || []).map(careUrl).filter(Boolean).slice(0,3);
...
function careCompare(r,id) {
  return `<div class="compare"><div class="photo-tabs" role="tablist" aria-label="입고 완료 사진 비교"><button role="tab" aria-selected="true" aria-controls="${id}-before" id="${id}-tab-before" data-photo="before">Before · 입고</button><button role="tab" aria-selected="false" tabindex="-1" aria-controls="${id}-after" id="${id}-tab-after" data-photo="after">After · 완료</button></div><div role="tabpanel" id="${id}-before" aria-labelledby="${id}-tab-before">${carePhotos(r.before_photos,'입고 사진')}</div><div role="tabpanel" id="${id}-after" aria-labelledby="${id}-tab-after" hidden>${carePhotos(r.after_photos,'완료 사진')}</div></div>`;
}
function careBindTabs() {
  document.querySelectorAll('.photo-tabs').forEach(group=>{
    const tabs=[...group.querySelectorAll('button')];
    const activate=tab=>tabs.forEach(t=>{ const on=t===tab; t.setAttribute('aria-selected',String(on));t.tabIndex=on?0:-1;document.getElementById(t.getAttribute('aria-controls')).hidden=!on; });
    tabs.forEach(t=>{t.onclick=()=>activate(t);t.onkeydown=e=>{if(['ArrowLeft','ArrowRight','Home','End'].includes(e.key)){e.preventDefault();const next=e.key==='Home'?tabs[0]:e.key==='End'?tabs[1]:tabs.find(x=>x!==t);activate(next);next.focus();}};});
  });
}
function careDetails(r,id) {
...
function renderCarePage(payload) {
  ...
  const completed=records.filter(r=>r.completed_on||careStage(r)==='출고');
  ...
  app.innerHTML=`<div class="care-public"><section class="care-hero"><p class="eyebrow">CARE RECORD · 케어 이력</p><div class="hero-top"><span class="category">${careText(item.item_type)}</span><span class="grade">${item.care_grade?esc(item.care_grade):'케어등급 미등록'}</span></div><h1>${esc(item.brand||item.item_type+' 케어 기록')}</h1><p class="model">${esc(item.model_name||'브랜드 · 모델명 미등록')}</p><p class="care-code">관리번호 <strong>${esc(item.public_code)}</strong></p><dl class="care-stats"><div><dt>누적 케어</dt><dd>${completed.length}<small>회</small></dd></div><div><dt>최근 케어일</dt><dd class="stat-text">${recent?esc(recent):'완료 이력 없음'}</dd></div><div><dt>담당자</dt><dd class="stat-text">${esc(current.staff_name||'담당자 배정 전')}</dd></div></dl></section><section class="care-section"><div class="section-heading"><h2>케어 진행상태</h2><span class="tag">${records.length?esc(stage):'이력 없음'}</span></div><ol class="progress-steps" aria-label="케어 진행상태">${CARE_STEPS.map((s,i)=>`<li class="${records.length&&i<=CARE_STEPS.indexOf(stage)?'reached':''}" ${records.length&&s===stage?'aria-current="step"':''}><span>${String(i+1).padStart(2,'0')}</span>${s}</li>`).join('')}</ol></section><section class="care-section"><div class="section-heading"><h2>이번 케어</h2><span class="muted">${esc(current.received_on||'')}</span></div>${records.length?careDetails(current,'current'):'<p class="muted">아직 등록된 케어가 없습니다.</p>'}</section><section class="care-section"><div class="section-heading"><h2>누적 이력</h2><span class="muted">${records.length}건의 기록</span></div><div class="timeline">${records.map((r,i)=>`<details><summary><span class="timeline-dot"></span><time>${esc(r.received_on||'날짜 미등록')}</time><span class="tag">${esc(careStage(r))}</span><span class="timeline-summary">${esc((r.processes?.length?r.processes.map(p=>p.name):r.services||[]).join(' · ')||'공정 등록 예정')}</span><span class="muted">담당 ${careText(r.staff_name)}</span>${carePhotos((r.after_photos?.length?r.after_photos:r.before_photos||[]).slice(0,1),'케어 썸네일')}</summary><div class="timeline-detail">${careDetails(r,`history-${i}`)}</div></details>`).join('')||'<p class="muted">누적 이력이 없습니다.</p>'}</div></section><footer class="care-footer"><p class="eyebrow">NEXT CARE</p><h2>다음 케어도, 안심하고.</h2>${booking?`<a class="booking" href="${esc(booking)}" target="_blank" rel="noopener noreferrer">네이버로 예약하기 ↗</a>`:'<p class="muted">예약 채널 준비 중</p>'}${settings.warranty_text?`<p class="warranty">${esc(settings.warranty_text)}</p>`:''}<div class="business">${settings.business_name?`<strong>${esc(settings.business_name)}</strong>`:''}${settings.business_owner?`<p>대표 ${esc(settings.business_owner)}</p>`:''}${settings.business_number?`<p>사업자등록번호 ${esc(settings.business_number)}</p>`:''}${settings.business_address?`<p>${esc(settings.business_address)}</p>`:''}</div><p class="privacy-note">고객 이름과 전화번호는 공개하지 않습니다.</p></footer></div>`;
  careBindTabs();
}
...
function careEditor(item,r={}) {
  ...
  out.innerHTML=`<section class="card"><h2>${esc(item.public_code)} · ${r.id?'케어 수정':'새 케어 접수'}</h2><p class="notice">공개 항목에 고객 이름·전화번호를 적지 마세요. 사진도 개인정보가 없는 이미지를 사용하세요.</p><label for="care-stage">현재 단계</label><select id="care-stage">${CARE_STEPS.map(s=>`<option ${s===careStage(r)?'selected':''}>${s}</option>`).join('')}</select><div class="grid"><div>${careInput('care-received','접수일',r.received_on||careDate(),'date')}</div><div>${careInput('care-completed','완료일',r.completed_on,'date')}</div></div>${careInput('edit-category','카테고리 (유모차 / 카시트)',item.item_type)}${careInput('edit-brand','브랜드',item.brand)}${careInput('edit-model','모델명',item.model_name)}${careInput('care-grade','케어등급 (제품 전체에 적용)',item.care_grade)}${careExtraFields(r,'edit')}<label for="care-before">입고 사진 추가 (합계 최대 3장)</label><input type="file" accept="image/*" multiple id="care-before">${carePhotos(r.before_photos,'기존 입고 사진')}<label for="care-after">완료 사진 추가 (합계 최대 3장)</label><input type="file" accept="image/*" multiple id="care-after">${carePhotos(r.after_photos,'기존 완료 사진')}${careArea('care-private','내부 메모 (관리자 전용)',r.notes)}<button id="save-care">저장</button><button class="secondary" id="cancel-care">돌아가기</button><p id="care-message" role="status"></p></section>`;
  ...
  out.querySelector('#save-care').onclick=async()=>{
    ...
    try {
      ...
      const received=document.querySelector('#care-received').value,stage=document.querySelector('#care-stage').value;
      const completed=document.querySelector('#care-completed').value||(stage==='출고'?careDate():null);
      if(!received||(completed&&completed<received))throw Error('접수일과 완료일을 확인하세요.');
      const beforeFiles=document.querySelector('#care-before').files,afterFiles=document.querySelector('#care-after').files;
      if((r.before_photos||[]).length+beforeFiles.length>3||(r.after_photos||[]).length+afterFiles.length>3)throw Error('입고·완료 사진은 각각 최대 3장입니다.');
      const before=[...(r.before_photos||[]),...await upload(beforeFiles,item.public_code,'before')],after=[...(r.after_photos||[]),...await upload(afterFiles,item.public_code,'after')];
