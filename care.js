...
function renderCarePage(payload) {
  ...
  app.innerHTML=`<div class="care-public"><section class="care-hero"><p class="eyebrow">${esc(settings.business_name||'세탁플랜')} · 케어 이력</p><h1>${esc(item.brand||'브랜드 미등록')}</h1><p class="model">${careText(item.item_type)}</p><p class="care-code">관리번호 <strong>${esc(item.public_code)}</strong></p><dl class="care-stats"><div><dt>누적 케어</dt><dd>${records.length}<small>회</small></dd></div><div><dt>첫 케어</dt><dd class="stat-text">${first?careFmt(first):'—'}</dd></div><div><dt>최근 케어</dt><dd class="stat-text">${records[0]?careFmt(records[0].received_on):'—'}</dd></div></dl></section><section class="care-section"><div class="section-heading"><h2>케어 기록</h2><span class="muted">${records.length}건</span></div><ul class="care-list">${records.map(r=>{const photos=carePhotoList(r).slice(0,3);const services=careServices(r);return `<li><time>${careFmt(r.received_on)}</time><div class="care-body"><p class="care-services">${services.length?esc(services.join(' · ')):'케어 완료'}</p>${careGallery(photos,'케어 사진')}</div></li>`}).join('')||'<li class="muted">아직 등록된 케어가 없습니다.</li>'}</ul></section><footer class="care-footer"><p class="eyebrow">NEXT CARE</p><h2>다음 케어도, 안심하고.</h2>${booking?`<a class="booking" href="${esc(booking)}" target="_blank" rel="noopener noreferrer">네이버로 예약하기 ↗</a>`:'<p class="muted">예약 채널 준비 중</p>'}${settings.warranty_text?`<p class="warranty">${esc(settings.warranty_text)}</p>`:''}<div class="business">${settings.business_name?`<strong>${esc(settings.business_name)}</strong>`:''}${settings.business_owner?`<p>대표 ${esc(settings.business_owner)}</p>`:''}${settings.business_number?`<p>사업자등록번호 ${esc(settings.business_number)}</p>`:''}${settings.business_address?`<p>${esc(settings.business_address)}</p>`:''}</div><p class="privacy-note">고객 이름과 전화번호는 공개하지 않습니다.</p></footer></div>`;
}
async function careCustomer(code) {
  document.body.classList.add('public-page');
  if(!code){app.innerHTML='<section class="care-hero"><p class="eyebrow">세탁플랜</p><h1>케어의 모든 기록.</h1><p class="muted">제품의 QR 코드를 스캔하면 케어 이력을 확인할 수 있습니다.</p></section>';return;}
...
function careEditor(item,r={}) {
  ...
  out.querySelector('#save-care').onclick=async()=>{
    ...
    try {
      ...
      const record={item_id:item.id,received_on:received,completed_on:received,status:'완료',services:careChecked(out),processes:[],photos:[...keep,...added],before_photos:[],after_photos:[],notes:out.querySelector('#care-private').value};
      ...
      const itemValues={brand:out.querySelector('#edit-brand').value.trim()};const update=await sb.from('items').update(itemValues).eq('id',item.id).select('id').single();if(update.error)throw update.error;
